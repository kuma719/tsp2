...
// // Anonymous sign-in (so rules can require auth)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  authBtn.addEventListener("click", async () => {
    ...
    if (user) {
      await auth.signOut();
      // サインアウト後にUI更新は onAuthStateChanged で行われる
    } else {
      try {
...
// 状態監視：UIテキストを切り替える＆保存ボタンなどの制御に使える
auth.onAuthStateChanged((user) => {
  const authBtn = document.getElementById("authBtn");
  if (authBtn) {
    authBtn.textContent = user ? "ログアウト" : "Googleでログイン";
  }
  // 例：ログイン必須の操作にガードを掛ける場合
...
// helper: upload to Storage and return {url, path, contentType}
async function uploadToStorage(path, fileOrBlob, contentType) {
  const ref = storage.ref().child(path);
  const task = await ref.put(fileOrBlob, { contentType });
...
function MemoryApp() {
  ...
  const audioBlobRef = useRef(null); // <— keep the recorded blob for upload

  // Keep track of user-selected files for upload (id -> File)
  const fileMapRef = useRef(new Map());

  // Hydrate Lucide icons after each render
  useEffect(() => {
  ...
  // Save the current page state into Firestore + Storage
  const saveMemory = async () => {
    const user = auth.currentUser;
    if (!user) {
      alert("サインイン中です。数秒後にもう一度お試しください。");
      return;
    }
    setSaving(true);
    try {
      // Create a new doc id first
      const docRef = db.collection("memories").doc();
      const basePath = `memories/${user.uid}/${docRef.id}`;

      // Upload newly-added media files (those present in fileMapRef)
      ...
      for (const m of media) {
        if (fileMapRef.current.has(m.id)) {
          ...
          uploadPromises.push(
            uploadToStorage(path, f, f.type).then((res) => {
              uploadedMedia.push({
                id: m.id,
                type: m.type,
                url: res.url,
          ...
          // External demo media — you can skip or include as external refs
          uploadedMedia.push({
            id: m.id,
            type: m.type,
            url: m.src,
      ...
      if (audioBlobRef.current) {
        ...
        uploadPromises.push(
          uploadToStorage(audioPath, audioBlobRef.current, "audio/webm").then((res) => {
            uploadedMedia.push({
              id: `a${Date.now()}`,
              type: "audio",
              url: res.url,
      ...
      alert("保存しました！");
      // Clear local file map since they're uploaded
      ...
      <div className="sticky top-0 z-20 backdrop-blur supports-[backdrop-filter]:bg-white/40 bg-white/70 border-b border-white/60">
        <div className="mx-auto max-w-6xl px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <button id="authBtn" className="px-3 py-1 rounded-full border border-black/10 hover:bg-black/5 transition">
              Googleでログイン
            </button>
            <button
              onClick={saveMemory}
              className={`px-3 py-1 rounded-lg border border-black/10 hover:bg-black/5 transition ${saving ? "opacity-60 cursor-not-allowed" : ""}`}
              disabled={saving}
              title="Firestoreに保存"
            >
              <div className="flex items-center gap-1">
